...
import preferences from 'hterm/preferences';
...
Terminal.prototype.getChangeObservers = function() {
  return preferences.getChangeObserversFor(this);
};
...
Terminal.prototype.setFontSize = function(px) {
  if (px === 0) {
    px = this.defaultFontSize_;
  } else {
    this.defaultFontSize_ = px;
  }
...
Terminal.prototype.eraseToLeft = function() {
  ...
  this.screen_.overwriteString(util.f.getWhitespace(cursor.column + 1));
  this.restoreCursor(cursor);
};
...
Terminal.prototype.eraseToRight = function(opt_count) {
  ...
  var cursor = this.saveCursor();
  this.screen_.overwriteString(util.f.getWhitespace(count));
  this.restoreCursor(cursor);
  this.clearCursorOverflow();
};
...
Terminal.prototype.insertSpace = function(count) {
  ...
  var ws = util.f.getWhitespace(count || 1);
...
Terminal.prototype.deleteChars = function(count) {
  ...
  if (deleted && !this.screen_.textAttributes.isDefault()) {
    ...
    this.screen_.insertString(util.f.getWhitespace(deleted));
    this.restoreCursor(cursor);
  }
...
Terminal.prototype.ringBell = function() {
  ...
  if (this.desktopNotificationBell_ && !this.document_.hasFocus()) {
    var n = new Notification(
        util.f.replaceVars(i18n.msg.desktopNotificationTitle,
