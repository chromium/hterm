...
import MockNotification from 'test/mock_notification';
import TestManager from 'test/test_manager';

export var TerminalTests = new TestManager.Suite('TerminalTests');
export default TerminalTests;

TerminalTests.prototype.setup = function(cx) {
  this.setDefaults(cx,
 ...
 */
TerminalTests.prototype.preamble = function(result, cx) {
  ...
  this.origNotification_ = window.Notification;
 ...
 */
TerminalTests.prototype.postamble = function(result, cx) {
  window.Notification = this.origNotification_;
};
 ...
 * actual test.
 */
TerminalTests.addTest = function(name, callback) {
...
TerminalTests.addTest('dimensions', function(result, cx) {
    var i;
    for (i = 0; i < this.visibleColumnCount; i++) {
    ...
    var divSize = util.dom.getClientSize(this.div);
 ...
 */
TerminalTests.addTest('plaintext-stress-cursor-ltr',
                      function(result, cx) {
 ...
 * code.
 */
TerminalTests.addTest('plaintext-stress-cursor-rtl',
                      function(result, cx) {
 ...
 * log is useful.
 */
TerminalTests.addTest('plaintext-stress-insert',
                      function(result, cx) {
    var chunkSize = 1000;
    var testCount = 10;
    var self = this;
