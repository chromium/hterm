 ...
 *
 * @param {TestManager.Log} opt_log Optional TestManager.Log object.
 *     Logs to the JavaScript console if ommitted.
 */
export var TestManager = function(opt_log) {
...
TestManager.Suite = function(suiteName) {
  ...
  ctor.testList_ = [];
  ctor.testMap_ = {};
  ctor.prototype = Object.create(TestManager.Suite.prototype);
 ...
 * Any exception here will abort the remainder of the test run.
 *
 * @param {TestManager.Result} result The result object for the upcoming
 *     test.
 * @param {Object} cx The context object for a test run.
 */
 ...
 *
 * These are created as part of the TestManager.Suite.addTest() method.
 * You should never have to construct one by hand.
 *
 * @param {TestManager.Suite} suiteClass The test suite class containing
 *     this test.
 * @param {string} testName The local name of this test case, not including the
 *     test suite name.
 * @param {function(TestManager.Result, Object)} testFunction The function
...
TestManager.TestRun = function(testManager, cx) {
   ...
   * reached.  If 0, the entire set of selected tests is run, even if some fail.
   */
  this.maxFailures = 1;
 ...
 * be selected.
 */
TestManager.TestRun.prototype.ALL_TESTS = ['<all-tests>'];
...
TestManager.TestRun.prototype.selectSuite = function(
...
TestManager.TestRun.prototype.selectPattern = function(pattern) {
  ...
  for (var i = 0; i < TestManager.Suite.subclasses.length; i++) {
    selectCount += this.selectSuite(TestManager.Suite.subclasses[i],
                                    pattern);
  }
 ...
 *     this function.
 */
TestManager.TestRun.prototype.onTestRunComplete_ = function(
    opt_skipTimeout) {
  if (!opt_skipTimeout) {
 ...
 * log defined by the associated TestManager.  By default this is
 ...
 *
 * Any failures in TestManager.* code or test suite setup or test case
 ...
 *     of the caller.
 */
TestManager.Result.prototype.assertEQ = function(
 ...
 * This will throw a TestCompleted exception, causing the current test to stop.
 */
TestManager.Result.prototype.pass = function() {
